...
namespace LinkuraLocal::Config {
    ...
    void LoadConfig(const std::string& configStr) {
        try {
            ...
            GetConfigItem(windowsInputTcpPort);
            if (localeCode != "ja-JP") {
                enableLocale = true;
            }
        }
    ...
    void UpdateConfig(const linkura::ipc::ConfigUpdate& configUpdate) {
        try {
            ...
            if (configUpdate.update_type() == linkura::ipc::ConfigUpdateType::FULL_UPDATE) {
                ...
                if (configUpdate.has_windows_input_tcp_port()) windowsInputTcpPort = configUpdate.windows_input_tcp_port();
